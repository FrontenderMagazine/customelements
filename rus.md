# Свои собственные элементы: как определять новые элементы в HTML

**Внимание!** В этой статье обсуждаются API, которые еще не полностью стандартизированы
и которые еще вполне могут измениться. Будьте осторожны с использованием э
кспериментальных API внутри своих проектов.

## Введение

Вебу недостает выразительной силы. Это легко проиллюстрировать — вот, посмотрите
на «современное» веб-приложение вроде GMail:

![Gmail][1] Современные веб-приложения разработаны супом из `<div>`ов.

В супе из `<div>`ов нет ничегошеньки современного. А все-таки вот так мы сейчас
...
ограничен лишь элементами, определенными в [стандарте HTML ][2].
...
Кастомные элементы, придите к нам на помощь! Вы — наша единственная надежда!
...
Здесь к вашим элементам можно добавлять кастомную функциональность (например,
публичные свойства и методы). [Об этом позже][6].
...
его можно поставить в пространство имен (`var myapp = {}; myapp.XFoo = document.register('x-foo');`)
или просто бросить на пол.

### Расширение встроенных элементов

Допустим, вы недовольны обычной, простой кнопкой `button`. Вы бы хотели, чтобы
ее возможности серьезно расширились, и чтобы кнопка стала мегакнопкой. Для этого,
...
Для HTML-элементов, которые не определны в этой спецификации, должен использоваться
...
<table>
<tr><td>Название</td><td>Наследует от</td><td>Примеры</td></tr>
<tr><td>Неопознанный элемент</td><td>HTMLElement</td><td>`<x-tabs>`, `<my-element>`, `<my-awesome-app>`</td></tr>
<tr><td>Неизвестный элемент</td><td>HTMLUnknownElement</td><td>`<tabs>`, `<foo_bar>`</td></tr>
</table>
...
браузера, с которыми он может работать дальше. Браузер говорит: «Ну что же, в вас
 есть все качестве, которые я ищу в новом элементе. Я обещаю, что сделаю вас
...
Как вы видите, функция `document.createElement()` может принимать атрибут `is=""`
...
Самое интересное в кастомных элементах — то, что вы можете включать какую хотите
функциональность вместе с элементом, определив свойства и методы в определении
элемента. Это можно представить себе как способ создавать для своего элемента
публичный API.
...
прототипы так, как описано выше, вот более краткая версия этого же кода:
...
Первый формат позволяет использовать `[Object.defineProperty`][9] из ES5. Второй
позволяет использовать [get/set][10].
...
<table>
<tr><td>Название коллбэка</td><td>Вызывается, когда</td></tr>

<tr><td>createdCallback</td>
<td>создан экземпляр элемента</td></tr>

<tr><td>enteredDocumentCallback</td>
<td>экземпляр вставлен в документ</td></tr>

<tr><td>leftDocumentCallback</td>
<td>экземпляр удален из документа</td></tr>

<tr><td>attributeChangedCallback(attrName, oldVal, newVal)</td>
<td>был добавлен, удален или изменен атрибут</td></tr>
</table>
...
**Все коллбэки жизненного цикла необязательны**, но определяйте их тогда, когда
...
Ваш элемент никто не будет использовать, если он толстый и неуклюжий. Колбэки
жизненного цикла помогут вам быть правильным членом общества!
...
Shadow DOM дает кастомным элементам:
  ...
  1. Возможность прятать внутреннюю сторону своей реализации — тем самым
  пользователи не будут видеть внутреннюю часть реализации.
  2. [Изоляция стилей][13] — бесплатно!

Создавать элемент из Shadow DOM можно точно так же, как и создавать элемент
из разметки. Разница содержится в `createdCallback()`:


    var XFooProto = Object.create(HTMLElement.prototype);

    XFooProto.createdCallback = function() {
      // 1. Создаем теневой корневой элемент на элементе.
      var shadow = this.createShadowRoot();

      // 2. И заполняем его разметкой.
...
корневой элемент для `x-foo-shadowdom` и заполнил его рaзметкой. Если внутри
...
[HTML-шаублоны][14] — это еще один новый низкоуровневый API, который прекрасно
встраивается в мир кастомных элементов.
...
**Пример:** регистрируем элементы, созданный из `<template>` и Shadow DOM:
...
происходит:
  ...
  3. Все страшные детали элемета спрятаны в Shadow DOM
...
Как и в случае любого HTML-тэга, ваш кастомный тэг можно стилизовать селекторами:
...
Кроличья нора идет гораздо, _гораздо_ глубже, когда вы используете решения с
...
Стилизация Shadow DOM — огромная тема! Если вы хотите узнать ее лучше, советую
вам несколько моих других статей:
...
проапгрейдился и превратился в элемент, соответствующий его определению.
...
не к элементам, которые наследуют от `HTMLUnkownElement` (см. [Обновление элементов][19]).
    ...
    <style>
      ...
      /* после того, как опредление x-panel регистрируется, они становятся зелеными */
      x-panel {
        color: green;
        display: block;
        padding: 5px;
...
## История и браузерная поддержка
...
Пока что браузерная поддержка далека от совершенства, но есть несколько отличных
полифиллов:
...
декларативно регстрировать новые элементы:
...
Здесь более чем достаточно вещей для старта.
